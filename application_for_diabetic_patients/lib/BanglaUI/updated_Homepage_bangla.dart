...
import '/BanglaUI/meal_tracking_bangla.dart';
import '/BanglaUI/mood_tracking_bangla.dart';
import '/BanglaUI/updated_medicine_tracker_bangla.dart';

// Import gamification managers
import '/BanglaUI/updated_achievement_manager_bangla.dart';
...
class MedicineSchedule {
  ...
  final String medicineName;
  final String dosage;
  final String frequency; // e.g., "দৈনিক", "সাপ্তাহিক", "নির্দিষ্ট দিনগুলি"
  ...
  final List<String> daysOfWeek; // e.g., ["সোমবার", "বুধবার"] for specific days
  final bool isActive;
  MedicineSchedule({
    required this.id,
    required this.username,
  ...
  String get formattedSchedule {
    ...
    } else if (frequency == 'সাপ্তাহিক' && daysOfWeek.isNotEmpty) {
...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    _speakGreeting(); // Speak greeting on app start
  }
  ...
  Future<void> _speakGreeting() async {
    ...
    if (lastGreetingDate != todayDate) {
      ...
      String greeting = "সুপ্রভাত, আপনি আজ আপনার ওষুধ নিয়েছেন কি?";
  ...
  Future<void> _initGamification() async {
    ...
    await MissionManager.resetMissions(); // Ensure missions are reset weekly
  ...
  void _handleGlucoseCommand(String recognizedWords) async {
    ...
    if (glucoseValue.isEmpty) {
      setState(() {
        _geminiResponse = "আমি সঠিক গ্লুকোজ মান শুনতে পাইনি। দয়া করে কিছু বলুন যেমন 'গ্লুকোজ 120'";
      });
      return;
    }
  ...
  void _handleMoodCommand(String recognizedWords) async {
    ...
    if (detectedMood != null) {
      ...
      setState(() {
        _geminiResponse = "আমি মেজাজ চিনতে পারিনি। দয়া করে কিছু বলুন যেমন 'আমি খুশি অনুভব করছি'";
      });
    }
  }
  ...
  void _handleMealCommand(String recognizedWords) async {
    ...
    if (mealDescription.isNotEmpty) {
      ...
      setState(() {
        _geminiResponse = "আমি শুনতে পাইনি আপনি কি খেয়েছেন। দয়া করে কিছু বলুন যেমন 'আমি পাস্তা খেয়েছি'";
      });
    }
  }
    ...
    if (medicineSchedulesJsonString != null && medicineSchedulesJsonString.isNotEmpty) {
      try {
        ...
        for (var schedule in allSchedules) {
          ...
          if (schedule.frequency == 'সাপ্তাহিক' && schedule.daysOfWeek.isNotEmpty) {
            final String todayDay = DateFormat('EEEE').format(now); // e.g., "বুধবার"
  ...
  void _onItemTapped(int index) async {
    ...
    switch (index) {
      case 0: // Home - no push needed, already there or navigated back to
        return;
      case 1:
        pageToPush = const GlucoseEntryBanglaScreen();
        break;
      case 2:
        pageToPush = const JournalBanglaScreen();
        break;
      case 3:
        pageToPush = const MealTrackerBanglaHomePage();
        break;
      case 4:
        pageToPush = const MoodTrackerBanglaHomePage();
        break;
      case 5:
                ...
                child: Column(
                  children: [
                    ...
                    if (_confidenceLevel > 0)
                      Text(
                        "আত্মবিশ্বাস: ${(_confidenceLevel * 100).toStringAsFixed(1)}%",
                        style: const TextStyle(
                          fontSize: 16,
            ...
            Positioned(
              ...
              child: Center(
                child: Container(
                  ...
                  child: const Text(
                    "স্বাস্থ্য প্রশ্ন শোনার জন্য অপেক্ষা করছি...",
                    style: TextStyle(
                      fontSize: 20,
                      color: Colors.white,
                    ),
