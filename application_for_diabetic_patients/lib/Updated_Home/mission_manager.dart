...
class MissionManager {
  ...
  static Future<List<String>> getMissions() async {
    ...
    if (lastResetStr != null) {
      ...
      // Check if it's a new week (e.g., Monday and current date is past last reset date's week)
      // We calculate the number of days until the next Monday from lastReset.
      // If the current day is past that, it's a new week.
      final daysUntilNextMondayFromLastReset = (DateTime.monday - lastReset.weekday + 7) % 7;
      final nextMondayFromLastReset = lastReset.add(Duration(days: daysUntilNextMondayFromLastReset));

      if (now.isAfter(nextMondayFromLastReset) && now.weekday == DateTime.monday) {
        await resetWeeklyMissions(); // Reset if a new week has truly started
      }
    } else {
